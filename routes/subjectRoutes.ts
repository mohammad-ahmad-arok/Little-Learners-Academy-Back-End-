...
subjectRouter
  .route("/")
  .get(getAllSubjects)
  .post(upload.single("image"),uploadImage("image"), createSubjectValidator, createSubject);

  subjectRouter
  .route("/:id")
  .get(getSubjectValidator, getSubject)
  .put(upload.single("image"),uploadImage("image"),updateSubjectValidator, updateSubject)
